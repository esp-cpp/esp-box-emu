...
void loadstate(FILE *f)
{
	...
	un32 (*header)[2] = (un32 (*)[2])gbc_filebuf;
	un32 d;
	int irl = hw.cgb ? 8 : 2;
	int vrl = hw.cgb ? 4 : 2;
...
struct svar svars[] =
{
	...
	palofs = 4096 - 512;
	oamofs = 4096 - 256;
	memset(gbc_filebuf, 0, /*sizeof gbc_filebuf*/4096);
	...
	for (int j = 0; j < srl; ++j) {
		memcpy(gbc_filebuf, (void*)tmp, 4096);
		size_t count = fwrite(gbc_filebuf, 4096, 1, f);
