...
{
#ifdef TFE_ESPBOX
	// The platform supplies the 320x200 8-bit buffer (see vfb_setPlatformBuffer()).
	static u8* s_frameBuffer320x200 = nullptr;
#else
...
#ifdef TFE_ESPBOX
	void vfb_setPlatformBuffer(u8* buffer)
	{
		s_frameBuffer320x200 = buffer;
		if (s_width == 320 && s_height == 200)
		{
			s_curFrameBuffer = buffer;
		}
	}
