...
#ifdef TFE_ESPBOX
	// Platform supplied 320x200 8-bit framebuffer; must be set before vfb_setResolution().
	void vfb_setPlatformBuffer(u8* buffer);
	// Forget the current resolution so the next vfb_setResolution() re-initializes.
	void vfb_resetState();
#endif
