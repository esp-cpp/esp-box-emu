...
namespace TFE_MidiPlayer
{
	...
	static std::vector<MidiSample> s_sampleBuffer;
	static MidiSample* s_sampleBufferPtr = nullptr;
	...
	void synthesizeMidi(MidiSample* buffer, u32 stereoSampleCount, bool updateBuffer)
	{
	...
	TFE_THREADRET midiUpdateFunc(void* userData)
	{
		...
		while (runThread)
		{
			...
			MUTEX_UNLOCK(&s_mutex);
#ifdef TFE_ESPBOX
			// Sleep between updates so the thread doesn't spin (iMuse runs at ~144Hz, FreeRTOS tick is 1ms).
			TFE_System::sleep((s_midiCallback.callback && !isPaused) ? 2 : 50);
#endif
#ifdef TFE_ESPBOX
