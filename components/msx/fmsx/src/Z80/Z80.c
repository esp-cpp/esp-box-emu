...
#define FAST_RDOP
extern byte *ROMPage[];
INLINE byte msx_OpZ80(word A) { return(ROMPage[A>>13][A&0x1FFF]); }
#endif

#ifdef SPECCY
#define msx_RdZ80 RDZ80
// @@@ WrZ80() can't be inlined as it contains debugging stuff
//#define WrZ80 WRZ80
extern byte *Page[],*ROM;
INLINE byte msx_RdZ80(word A) { return(Page[A>>13][A&0x1FFF]); }
...
#define FAST_RDOP
extern byte *RAM[];
INLINE byte msx_OpZ80(word A) { return(RAM[A>>13][A&0x1FFF]); }
#endif

#ifdef ATI85
#define msx_RdZ80 RDZ80
#define msx_WrZ80 WRZ80
extern byte *Page[],*ROM;
extern void (*DoMEM)(register word Addr,register byte V);
INLINE byte msx_RdZ80(word A) { return(Page[A>>14][A&0x3FFF]); }
INLINE void msx_WrZ80(word A,byte V) { if(Page[A>>14]<ROM) Page[A>>14][A&0x3FFF]=V; else DoMEM(A,V); }
#endif
...
#define M_PUSH(Rg)     \
  msx_WrZ80(--R->SP.W,R->Rg.B.h);msx_WrZ80(--R->SP.W,R->Rg.B.l)

#define M_CALL         \
  J.B.l=OpZ80(R->PC.W++);J.B.h=OpZ80(R->PC.W++);         \
  msx_WrZ80(--R->SP.W,R->PC.B.h);msx_WrZ80(--R->SP.W,R->PC.B.l); \
  R->PC.W=J.W; \
  JumpZ80(J.W)
...
#define M_RST(Ad)      \
  msx_WrZ80(--R->SP.W,R->PC.B.h);msx_WrZ80(--R->SP.W,R->PC.B.l);R->PC.W=Ad;JumpZ80(Ad)
...
#include "Codes.h"
    ...
    else
    {
      ...
      if((R->IRequest!=INT_NONE)&&(R->IRequest!=INT_QUIT)) msx_IntZ80(R,R->IRequest);
    }
  }
}
#endif /* EXECZ80 */
...
/*************************************************************/
void msx_IntZ80(Z80 *R,word Vector)
{
  ...
  if((R->IFF&IFF_1)||(Vector==INT_NMI))
  {
    ...
    if(R->IFF&IFF_IM2)
    {
      ...
      /* Read the vector */
      R->PC.B.l=msx_RdZ80(Vector++);
      R->PC.B.h=msx_RdZ80(Vector);
      JumpZ80(R->PC.W);
      /* Done */
      return;
    }
...
#include "Codes.h"
    ...
    if(R->ICount<=0)
    {
      ...
      if(R->IFF&IFF_EI)
      {
        ...
        {
          J.W=msx_LoopZ80(R);        /* Call periodic handler    */
      ...
      if(J.W!=INT_NONE) msx_IntZ80(R,J.W);   /* Int-pt if needed */
    }
  /* } */
