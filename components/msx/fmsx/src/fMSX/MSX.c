...
Z80 *CPU = NULL;                           /* Z80 CPU state and regs */

byte *msx_VRAM,*VPAGE;                 /* Video RAM              */
...
void **Chunks = NULL;           /* Memory blocks to free  */
...
I8255 *PPI=NULL;                         /* i8255 PPI at A8h-ABh   */
byte IOReg;                        /* Storage for AAh port   */

/** Disk controller: WD1793 **********************************/
WD1793 *FDC=NULL;                        /* WD1793 at 7FF8h-7FFFh  */
FDIDisk *FDD = NULL;                    /* Floppy disk images     */

/** Sound hardware: PSG, SCC, OPLL ***************************/
AY8910 *PSG=NULL;                        /* PSG registers & state  */
YM2413 *OPLL=NULL;                       /* OPLL registers & state */
SCC  *SCChip=NULL;                       /* SCC registers & state  */
byte SCCOn[2];                     /* 1 = SCC page active    */
word FMPACKey;                     /* MAGIC = SRAM active    */

/** Serial I/O hardware: i8251+i8253 *************************/
I8251 *SIO=NULL;                         /* SIO registers & state  */
...
MCFEntry *MCFEntries = NULL;    /* Entries from .MCF file */
...
{
  // return (byte*)shared_malloc(Size);
  byte *P;
...
int ResetMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  ChrTab=ColTab=ChrGen=msx_VRAM;            /* VDP tables       */
  SprTab=SprGen=msx_VRAM;
  ChrTabM=ColTabM=ChrGenM=SprTabM=~0;   /* VDP addr. masks  */
  VPAGE=msx_VRAM;                           /* VRAM page        */
...
/** given I/O port.                                         **/
/*************************************************************/
void msx_OutZ80(word Port,byte Value)
{
  register byte I,J;
...
byte SetScreen(void)
{
  ...
  ChrTab  = msx_VRAM+((int)(VDP[2]&MSK[J].R2)<<I);
  ChrGen  = msx_VRAM+((int)(VDP[4]&MSK[J].R4)<<11);
  ColTab  = msx_VRAM+((int)(VDP[3]&MSK[J].R3)<<6)+((int)VDP[10]<<14);
  SprTab  = msx_VRAM+((int)(VDP[5]&MSK[J].R5)<<7)+((int)VDP[11]<<15);
  SprGen  = msx_VRAM+((int)VDP[6]<<11);
...
{ 
  ...
  switch(R)  
  {
    ...
    case  2: J=(ScrMode>6)&&(ScrMode!=MAXSCREEN+1)? 11:10;
             ChrTab  = msx_VRAM+((int)(V&MSK[ScrMode].R2)<<J);
             ChrTabM = ((int)(V|~MSK[ScrMode].M2)<<J)|((1<<J)-1);
             break;
    case  3: ColTab  = msx_VRAM+((int)(V&MSK[ScrMode].R3)<<6)+((int)VDP[10]<<14);
             ColTabM = ((int)(V|~MSK[ScrMode].M3)<<6)|0x1C03F;
             break;
    case  4: ChrGen  = msx_VRAM+((int)(V&MSK[ScrMode].R4)<<11);
             ChrGenM = ((int)(V|~MSK[ScrMode].M4)<<11)|0x007FF;
             break;
    case  5: SprTab  = msx_VRAM+((int)(V&MSK[ScrMode].R5)<<7)+((int)VDP[11]<<15);
             SprTabM = ((int)(V|~MSK[ScrMode].M5)<<7)|0x1807F;
             break;
    case  6: V&=0x3F;SprGen=msx_VRAM+((int)V<<11);break;
    case  7: FGColor=V>>4;BGColor=V&0x0F;break;
    case 10: V&=0x07;
             ColTab=msx_VRAM+((int)(VDP[3]&MSK[ScrMode].R3)<<6)+((int)V<<14);
             break;
    case 11: V&=0x03;
             SprTab=msx_VRAM+((int)(VDP[5]&MSK[ScrMode].R5)<<7)+((int)V<<15);
             break;
    case 14: V&=VRAMPages-1;VPAGE=msx_VRAM+((int)V<<14);
             break;
...
int ApplyCheats(void)
{
  ...
  for(J=I=0;J<CheatCount;++J)
    ...
    {
      msx_WrZ80(CheatCodes[J].Addr&0xFFFF,CheatCodes[J].Data&0xFF);
      if(CheatCodes[J].Size>1)
        msx_WrZ80((CheatCodes[J].Addr+1)&0xFFFF,CheatCodes[J].Data>>8);
      ++I;
    }
