...
****************************************************************/
{
  ...
  msx_OutZ80(0xA8,0xFF);
  SSlot(0xAA);

  if(R->AF.B.l&C_FLAG)
    for(Sector=R->DE.W;Count--;Sector++) /* WRITE */
    {
      for(J=0;J<512;J++) Buf[J]=msx_RdZ80(Addr++);
    ...
    for(Sector=R->DE.W;Count--;Sector++) /* READ */
    {
      ...
      for(J=0;J<512;J++) msx_WrZ80(Addr++,Buf[J]);
    }
...
****************************************************************/
{
  ...
  Addr=R->HL.W+1;
  msx_WrZ80(Addr++,Buf[0x15]);             /* Format ID [F8h-FFh] */
  msx_WrZ80(Addr++,Buf[0x0B]);             /* Sector size         */
  msx_WrZ80(Addr++,Buf[0x0C]);
  J=(BytesPerSector>>5)-1;
  for(I=0;J&(1<<I);I++);
  msx_WrZ80(Addr++,J);                     /* Directory mask/shft */
  msx_WrZ80(Addr++,I);
  J=Buf[0x0D]-1;
  for(I=0;J&(1<<I);I++);
  msx_WrZ80(Addr++,J);                     /* Cluster mask/shift  */
  msx_WrZ80(Addr++,I+1);
  msx_WrZ80(Addr++,Buf[0x0E]);             /* Sector # of 1st FAT */
  msx_WrZ80(Addr++,Buf[0x0F]);
  msx_WrZ80(Addr++,Buf[0x10]);             /* Number of FATs      */
  msx_WrZ80(Addr++,Buf[0x11]);             /* Number of dirent-s  */
  J=ReservedSectors+Buf[0x10]*SectorsPerFAT;
  J+=32*Buf[0x11]/BytesPerSector;
  msx_WrZ80(Addr++,J&0xFF);                /* Sector # of data    */
  msx_WrZ80(Addr++,(J>>8)&0xFF);
  J=(SectorsPerDisk-J)/Buf[0x0D];
  msx_WrZ80(Addr++,J&0xFF);                /* Number of clusters  */
  msx_WrZ80(Addr++,(J>>8)&0xFF);
  msx_WrZ80(Addr++,Buf[0x16]);             /* Sectors per FAT     */
  J=ReservedSectors+Buf[0x10]*SectorsPerFAT;
  msx_WrZ80(Addr++,J&0xFF);                /* Sector # of dir.    */
  msx_WrZ80(Addr,(J>>8)&0xFF);
