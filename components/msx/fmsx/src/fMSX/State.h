...
#define SaveSTRUCTPTR(Name, TYPENAME) \
  if(Size+sizeof(TYPENAME)>MaxSize) return(0); \
  else { memcpy(Buf+Size,(Name),sizeof(TYPENAME));Size+=sizeof(TYPENAME); }
...
#define LoadSTRUCTPTR(Name, TYPENAME) \
  if(Size+sizeof(TYPENAME)>MaxSize) return(0); \
  else { memcpy((Name),Buf+Size,sizeof(TYPENAME));Size+=sizeof(TYPENAME); }
...
unsigned int SaveState(unsigned char *Buf,unsigned int MaxSize)
{
  ...
  SaveSTRUCTPTR(CPU, Z80);
  SaveSTRUCTPTR(PPI, I8255);
  SaveSTRUCT(VDP);
  SaveARRAY(VDPStatus);
  SaveARRAY(Palette);
  SaveSTRUCTPTR(PSG, AY8910);
  SaveSTRUCTPTR(OPLL, YM2413);
  SaveSTRUCTPTR(SCChip, SCC);
  SaveARRAY(State);
  SaveDATA(RAMData,RAMPages*0x4000);
  SaveDATA(msx_VRAM,VRAMPages*0x4000);
...
unsigned int LoadState(unsigned char *Buf,unsigned int MaxSize)
{
  ...
  /* Load hardware state */
  LoadSTRUCTPTR(CPU, Z80);
  LoadSTRUCTPTR(PPI, I8255);
  LoadSTRUCT(VDP);
  LoadARRAY(VDPStatus);
  LoadARRAY(Palette);
  LoadSTRUCTPTR(PSG, AY8910);
  LoadSTRUCTPTR(OPLL, YM2413);
  LoadSTRUCTPTR(SCChip, SCC);
  LoadARRAY(State);
  LoadDATA(RAMData,RAMPages*0x4000);
  LoadDATA(msx_VRAM,VRAMPages*0x4000);
